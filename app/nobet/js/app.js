...
function nobetListesiOlustur() {
    ...
    setTimeout(() => {
        const ayarlar = {
            ...
            haftaSonuAyri: document.getElementById('haftaSonuAyri').checked,
            maxShifts: parseInt(document.getElementById('maxShifts').value) || 0
        };
    ...
    schedule.forEach((day, dayIdx) => {
        ...
        const assignments = day.assigned.map((p, pIdx) => {
            ...
            return `<span class="inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors group" 
                       ...
                       ${p.ad}${pIdx < day.assigned.length - 1 ? ',' : ''}
...
window.overrideAc = function(date, pIdx, id, ad) {
    ...
    personeller.forEach(p => {
        ...
        // Comprehensive date normalization check
    ...
    select.onchange = () => {
        ...
        if (isExcused) {
            ...
            warning.innerHTML = `<i class="ri-error-warning-fill"></i> <b>YASAK:</b> ${person.ad} bu tarihte mazeretli, nöbet yazılamaz!`;
            confirmBtn.disabled = true;
...
function handleDevamBtn() {
    if (mevcutAdim < adimlar.length - 1) {
        if (mevcutAdim === 0) {
            const start = new Date(document.getElementById('baslangicTarihi').value);
            const end = new Date(document.getElementById('bitisTarihi').value);
            
            if (!document.getElementById('baslangicTarihi').value || !document.getElementById('bitisTarihi').value) {
                    ...
                    mazeretData.forEach((m, idx) => {
                        ...
                        if (person) {
                            ...
                            if (mazeretTarihi instanceof Date) {
                                mazeretTarihi = mazeretTarihi.toISOString().split('T')[0];
                            } else if (typeof mazeretTarihi === 'string' && mazeretTarihi.includes('.')) {
                                // Handle DD.MM.YYYY
                                const parts = mazeretTarihi.split('.');
                                if (parts.length === 3) {
