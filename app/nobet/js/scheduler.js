...
const scheduler = {
    ...
    generate(personeller, baslangic, bitis, ayarlar) {
        ...
        const personState = personeller.map(p => ({
            id: p.id,
            ad: p.ad,
            // Historical load weights: Weekend is more "expensive" to maintain balance
            initialScore: (p.nobetSayisi * 1) + (p.haftaSonuNobetSayisi * 2),
            currentScore: 0,
            weekdayCount: 0,
            weekendCount: 0,
            totalAssigned: 0,
        ...
        const solve = (dayIndex) => {
            ...
            let candidates = personState.filter(p => {
                // 1. Mazeret Check
                if (p.mazeretler.has(currentDay.dateStr)) {
            ...
            candidates.sort((a, b) => {
                const scoreA = (a.initialScore + a.currentScore) + (currentDay.isWeekend ? a.weekendCount * 5 : a.weekdayCount);
                const scoreB = (b.initialScore + b.currentScore) + (currentDay.isWeekend ? b.weekendCount * 5 : b.weekdayCount);
